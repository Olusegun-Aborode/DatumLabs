...
Fluid's USDC supply APY fell from 6.41% at June 30 to 4.66% at July 31. Fluid still leads the sector on USDC supply, but the gap to Aave V3 (3.27%) narrowed from 322 bps to 139 bps. Fluid was the sector's only positive real-yield spread over T-bill at the June close (+281 bps); at the July close it sits at −107 bps below the T-bill.
...
Aave V3's July composition mirrored June. USDC contracted by $20M across the month. The remaining +$669M came from collateral inflows across the LRT and BTC-variant reserves, with the largest additions in weETH (+$285M), sUSDe (+$145M), and WBTC (+$124M). The observation from Issue 003 continues to hold: Aave V3's growth mechanism in this cycle is collateral supply attracted by borrow-book depth, not stablecoin depositors chasing rates. Two consecutive months of the same pattern is now a trend, not a June anomaly.
...
SparkLend's July flow was −$203M, the largest single-protocol contraction of the month. Five of SparkLend's eighteen reserves price their borrow rate off a single external anchor rather than off pool utilization; all five are stablecoins, and all five stepped together on July 23. At 14:43 UTC that day, Sky's weekly Atlas Edit executive spell executed on-chain, cutting the Sky Base Rate (the wholesale cost of capital SparkLend pays for its USDS inventory) from 3.90% to 3.72%. SparkLend's on-chain USDC borrow rate followed the same day, stepping from 4.42% to 4.28%.
...
SparkLend is therefore not a single-mechanism protocol. It is a hybrid: a Sky-linked stablecoin book (DAI, USDC, USDT, USDS, PYUSD, all pointing at a single Sky-linked rate source) bolted onto a standard Aave-style collateral book of thirteen reserves that clear on utilization curves. Comparing Fluid's 4.66% supply APY on USDC to SparkLend's 3.47% is not a fair like-for-like: the two rates are set by different mechanisms responding to different inputs.
