---
title: The State of Lending on Ethereum — April 2026
slug: state-of-lending-ethereum-april-2026
period: April 2026
publishedAt: 2026-05-07T00:01:00.000Z
summary: A single LayerZero bridge exploit drained Kelp's rsETH adapter on April 18 and cascaded through Ethereum lending markets in 96 hours. Aave V3 shed $14.57B in deposits, Spark absorbed the wstETH migration, and the rsETH bad-debt question remains unresolved. The full April view across Aave V3, SparkLend, Morpho, and Fluid on Ethereum mainnet.
excerpt: A bridge exploit, a $14.57B Aave outflow, and a bad-debt question that April did not settle.
cover: /images/reports/state-of-lending-ethereum-issue-001.png
gated: false
---

<KPIGrid>
  <KPI label="Sector TVL" value="−23.0%" sub="$41.91B → $32.26B in one month" />
  <KPI label="Aave V3 alone" value="−$14.57B" sub="largest single-month outflow on record" />
  <KPI label="Spark" value="+73%" sub="only protocol with positive net flows" />
  <KPI label="Implied bad debt" value="$124M-$230M" sub="Kelp rsETH allocation pending" />
</KPIGrid>
...
LTV refers to loan-to-value, the maximum debt allowed against a unit of collateral. The IRM (interest rate model) is the formula each protocol uses to set borrow rates as a function of utilization, with a "kink" inflection point above which rates rise much more steeply.
...
| Protocol | Net deposits 30d | Apr 30 supply | MoM change |
|---|---:|---:|---:|
...
| Protocol | Mar 2026 | Apr 2026 | MoM Δ | MoM % |
|---|---:|---:|---:|---:|
...
| Indicator | Reading |
|---|---|
| Stablecoin debt share | 43.6% (down from 48.3% in March) |
| Oracle concentration | Chainlink 94.0%, Redstone 4.5%, other ~1.5% |
| Liquidation intensity 90D | Fluid 10.0%, Aave V3 2.2%, Morpho 0.5%, Spark 0.04% |
| Liquidation efficiency | Fluid 1.92%, Spark ~5%, Aave V3 ~6%, Morpho ~18% (effective penalty) |
| wstETH largest 30d move | +$980M (Spark), −$1.39B (Aave V3) |
| Cross-protocol USDC supply APY dispersion | 833 bps (12-month avg: 232 bps) |
| Morpho curator HHI | 3,026 (highly concentrated; up from 2,700 in March) |

## Executive summary

<Callout tone="brand">April was the month an attacker drained Kelp's rsETH bridge adapter and used the stolen tokens as collateral on Aave V3.</Callout>
...
<Chart type="line" title="Real Yield Spread across 12 months: April closes at −28 bps, closest to parity since the inversion began" x="month"
  series={["Spread (bps)"]}
  data={[
...
<Chart type="bar" title="Sector at April 30: Aave V3 supply and borrows dominate the four-protocol coverage" x="protocol"
  series={["Supplied $B","Borrowed $B"]}
  data={[
    { protocol: "Aave V3", "Supplied $B": 20.41, "Borrowed $B": 9.26 },
    { protocol: "Morpho", "Supplied $B": 5.79, "Borrowed $B": 2.16 },
    { protocol: "SparkLend", "Supplied $B": 5.11, "Borrowed $B": 1.64 },
    { protocol: "Fluid", "Supplied $B": 1.08, "Borrowed $B": 0.57 }
  ]} />
...
<Chart type="bar" title="April net flow by protocol: Aave V3 shed $14.6B; SparkLend was the only positive protocol" x="protocol"
  series={["Net flow $M"]}
  data={[
    { protocol: "Aave V3", "Net flow $M": -14570 },
    { protocol: "SparkLend", "Net flow $M": 1970 },
    { protocol: "Morpho", "Net flow $M": -464 },
    { protocol: "Fluid", "Net flow $M": -113 }
  ]} />
...
<Chart type="bar" title="Loan-to-Deposit Ratio at April 30: Fluid runs highest, SparkLend lowest" x="protocol"
  series={["LDR %"]}
  data={[
    { protocol: "Fluid", "LDR %": 53.19 },
    { protocol: "Aave V3", "LDR %": 45.39 },
    { protocol: "Morpho", "LDR %": 37.37 },
    { protocol: "SparkLend", "LDR %": 32.03 }
  ]} />
...
## Theme: The curator concentration
...
| Protocol | Volume / TVL | Volume (90D) |
|---|---:|---:|
| Fluid | 10.0% | $52.22M |
| Aave V3 | 2.2% | $256.07M |
| Morpho | 0.5% | $13.69M |
...
- Wallet-level position data is not yet in coverage; health-factor distributions and top-borrower concentration are not in this issue.
...
- Snapshot timestamp: April 30, 2026, 23:59 UTC. Time-series data covers June 2024 through May 5, 2026.

---

*Figures as of 30 April 2026, 23:59 UTC, unless otherwise noted. Data draws on the DefiLlama Yields and Fees APIs, on-chain reads of Aave V3 and SparkLend reserve configurations and live state via UiPoolDataProviderV3, the Morpho API for vault-level data, the Fluid subgraph, the Datum Labs Liquidator Economy database, the FRED DFF series for the 4-week T-bill rate, and the Aave incident report dated April 20, 2026 for event-specific details on the Kelp rsETH bridge exploit.*
