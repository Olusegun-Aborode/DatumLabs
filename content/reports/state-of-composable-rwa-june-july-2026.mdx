...
<KPIGrid>
  <KPI label="Posted as collateral" value="$523.1M" sub="Aave Horizon + Morpho, 31 July" />
  <KPI label="Actually used, market-wide" value="under $2B" sub="of ~$38B tokenized RWA on-chain" />
  <KPI label="USCC wind-down" value="−90%" sub="10.5M to 1.07M tokens, at full value" />
  <KPI label="Morpho min health factor" value="below 1.0" sub="every day from 4 July, no liquidation" />
</KPIGrid>

*As of 31 July 2026. Our own numbers are first-party, drawn from the Datum Labs RWA Terminal's daily store and reconciled to DefiLlama within 0.02% at the 2 July audit. We track two venues where tokenized real-world assets are used as collateral on Ethereum: Aave Horizon and Morpho's RWA markets. Where we could not verify something on-chain, we left it out and said so, and the gaps are listed in the methodology. Outside data and quotes, including sources that disagree with us, are cited throughout and listed at the end.*
